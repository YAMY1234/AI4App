...
from postprocessing.tools.replace_constant import remove_specific_text, replace_specific_text
...
def external_dispose(school_abbr):
    ...
    replace_standard_external_caller(school_abbr=school_abbr, col_name_list=external_replace_col_list,
                                     standard_path="/Users/liyangmin/Documents/创业/英硕项目/格拉斯哥- 工作经验修复.xlsx")


def remove_text_dispose(school_abbr):
    col_name_list = [header.course_duration_1]
    remove_word_list = [" calendar year (full-time)"]
    remove_specific_text(school_abbr, col_name_list, remove_word_list)


def replace_text_dispose(school_abbr):
    col_name_list = [header.course_duration_1]
    remove_word_list = [
        "academic years \(full-time\)",
        "academic year \(full-time\)",
        "calendar years \(full-time\)",
        "calendar year \(full-time\)",
        "academic years \(part-time\)",
        "academic year \(part-time\)",
        "calendar years \(part-time\)",
        "calendar year \(part-time\)",
        "calendar years \(flexible\)",
        "Academic years \(flexible\)",
        "calendar year \(flexible\)",
        "Academic year \(flexible\)",
        "Years \(flexible\)",
        "terms \(full-time\)",
        "months \(full-time\)",
        "month \(full-time\)",
    ...
    replace_specific_text(school_abbr, col_name_list, remove_word_list, replace_word_list)



def replace_text_dispose_entry_month(school_abbr):
    col_name_list = [header.admission_month_1]
    remove_word_list = [
        "September 2024"
    ]
    replace_word_list = [
...
def remove_course_fee_unit(school_abbr):
    col_name_list = [header.admission_month_1]
    remove_word_list = [
        "£"
    ]
...
def change_未要求_to_官网没有明确说明(school_abbr):
    col_name_list = [header.admission_month_1, header.work_experience_details, header.work_experience_years, header.portfolio, header.gre, header.gmat]
    remove_word_list = [
        "未要求",
        "可能要求"
    ]
...
def replace_already_translated(school_abbr):
    standard_path = "/Users/liyangmin/PycharmProjects/AI4App/data/UCL/program_details_gpt.xlsx"
    external_replace_col_list = [
        header.course_list_chinese,
        header.background_requirements_chinese,
        header.project_intro_chinese,
    ]
    replace_standard_external_caller(school_abbr=school_abbr, col_name_list=external_replace_col_list, standard_path=standard_path)


def UCL_post_dispose():
    # STAGE2
    # dispose_major_caller(school_abbr=school_abbr)
    #
    ...
    #
    # replace_text_dispose(school_abbr=school_abbr)
    # replace_text_dispose_entry_month(school_abbr=school_abbr)
    # remove_course_fee_unit(school_abbr=school_abbr)
    # change_未要求_to_官网没有明确说明(school_abbr=school_abbr)
    ...
    #     (header.course_list_english, header.course_list_chinese),
    #     (header.background_requirements, header.background_requirements_chinese),
    #     (header.project_intro, header.project_intro_chinese),
    # ]
    # translate_api_caller(school_abbr=school_abbr, col_name_tuple_list=translate_col_tuple_list)

    replace_already_translated(school_abbr)
