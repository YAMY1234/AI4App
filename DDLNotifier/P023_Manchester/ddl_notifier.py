...
def get_deadline(url):
    ...
    try:
        ...
        if not staged_admissions_header:
            return "Staged admissions section not found."

        # Find the div containing the staged admissions details
        staged_admissions_div = staged_admissions_header.find_next_sibling("div")
        if staged_admissions_div:
            texts = ' '.join(text.strip() for text in staged_admissions_div.find_all(text=True))
            return texts
        else:
            return "Staged admissions details not provided"
